...
sys.path.insert(0, os.path.join(CLEAN_ROOT, 'main_route_classifier', 'src'))
...
RANDOM_SEARCH_SPACE: Dict[str, list] = {'learning_rate': [1e-05, 2e-05, 3e-05, 5e-05], 'batch_size': [8, 16, 32], 'dropout_rate': [0.1, 0.2, 0.3], 'weight_decay': [0.001, 0.01, 0.05, 0.1], 'warmup_ratio': [0.0, 0.05, 0.1, 0.2], 'num_epochs': [10, 15, 20], 'lr_scheduler_type': ['cosine', 'linear'], 'max_length': [64, 128]}
GRID_SEARCH_SPACE: Dict[str, list] = {'learning_rate': [1e-05, 2e-05, 3e-05], 'batch_size': [8, 16], 'dropout_rate': [0.1, 0.2, 0.3], 'weight_decay': [0.01, 0.05], 'warmup_ratio': [0.1], 'num_epochs': [15, 20], 'lr_scheduler_type': ['cosine'], 'max_length': [128]}
...
def train_fold(hp: Dict[str, Any], train_texts: List[str], train_labels: List[int], val_texts: List[str], val_labels: List[int], tokenizer, device: torch.device, fold_idx: int, verbose: bool=False) -> Dict[str, Any]:
    ...
    class_weights = torch.tensor([total / (config.num_labels * label_counts.get(i, 1)) for i in range(config.num_labels)], dtype=torch.float32).to(device)
    ...
    for epoch in range(config.num_epochs):
        ...
        val_metrics = compute_metrics(np.array(all_probs), np.array(all_labels), config.label_names)
        val_metrics['loss'] = val_loss / max(len(val_loader), 1)
        if verbose:
            logging.info(f"    Fold {fold_idx + 1} | Epoch {epoch + 1:2d}/{config.num_epochs} | Train loss {avg_loss:.4f} | Val F1 {val_metrics['macro_f1']:.4f} | Val Acc {val_metrics['accuracy']:.4f}")
        ...
        if patience_counter >= EARLY_STOPPING_PATIENCE:
            ...
            break
    return {'best_val_f1': best_val_f1, 'best_val_accuracy': best_val_metrics.get('accuracy', 0.0), 'best_val_roc_auc': best_val_metrics.get('roc_auc', 0.0), 'best_epoch': best_epoch, 'epochs_run': epoch + 1, 'val_metrics': best_val_metrics}

def run_trial(trial_idx: int, hp: Dict[str, Any], texts: List[str], labels: List[int], n_folds: int, device: torch.device, verbose: bool=False) -> Dict[str, Any]:
    config = _make_config(hp)
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(skf.split(texts_arr, labels_arr)):
        ...
        fold_result = train_fold(hp, fold_train_texts, fold_train_labels, fold_val_texts, fold_val_labels, tokenizer, device, fold_idx, verbose=verbose)
    ...
    return {'trial': trial_idx + 1, 'hyperparams': hp, 'mean_val_f1': float(np.mean(f1_scores)), 'std_val_f1': float(np.std(f1_scores)), 'mean_val_accuracy': float(np.mean(acc_scores)), 'std_val_accuracy': float(np.std(acc_scores)), 'mean_val_roc_auc': float(np.mean(auc_scores)), 'std_val_roc_auc': float(np.std(auc_scores)), 'per_fold': fold_results}

def retrain_best(best_hp: Dict[str, Any], data_path: str, output_dir: str, device: torch.device):
    ...
    if config.lr_scheduler_type == 'cosine':
        ...
        scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=warmup_steps, num_training_steps=total_steps)
    logging.info(f'Retraining best config for {config.num_epochs} epochs on {len(merged_dataset)} samples (train+val merged)')
    for epoch in range(config.num_epochs):
        model.train()
        epoch_loss = 0.0
        for batch in train_loader:
    ...
    torch.save({'model_state_dict': model.state_dict(), 'config': config, 'tokenizer': tokenizer, 'hyperparams': best_hp, 'test_metrics': test_metrics}, model_path)
...
def main():
    parser = argparse.ArgumentParser(description='Hyperparameter search for headline persuasion-route classifier')
    ...
    parser.add_argument('--strategy', type=str, default='random', choices=['random', 'grid'], help="Search strategy: 'random' or 'grid'")
    parser.add_argument('--n_trials', type=int, default=30, help='Number of random trials (ignored for grid)')
    parser.add_argument('--n_folds', type=int, default=5, help='Number of CV folds (default: 5)')
    parser.add_argument('--seed', type=int, default=42, help='Random seed')
    parser.add_argument('--verbose', action='store_true', help='Print per-epoch per-fold logs')
    parser.add_argument('--retrain_best', action='store_true', help='After search, retrain the best config and evaluate on test set')
    ...
    for i, hp in enumerate(hp_configs):
        ...
        logging.info(f"\n{'=' * 60}\nTrial {i + 1}/{len(hp_configs)}\n  lr={hp['learning_rate']:.1e}  bs={hp['batch_size']}  dropout={hp['dropout_rate']}  wd={hp['weight_decay']}  warmup={hp['warmup_ratio']}  epochs={hp['num_epochs']}  sched={hp['lr_scheduler_type']}  maxlen={hp['max_length']}\n{'=' * 60}")
        result = run_trial(trial_idx=i, hp=hp, texts=texts, labels=labels, n_folds=args.n_folds, device=device, verbose=args.verbose)
        trial_time = time.time() - trial_start
        result['trial_time_seconds'] = trial_time
        all_results.append(result)
        logging.info(f"  → Mean Val F1: {result['mean_val_f1']:.4f} ± {result['std_val_f1']:.4f}  |  Acc: {result['mean_val_accuracy']:.4f} ± {result['std_val_accuracy']:.4f}  |  AUC: {result['mean_val_roc_auc']:.4f}  |  {trial_time:.1f}s")
...
def _save_search_results(all_results: List[Dict], best_trial: Optional[Dict], args, total_start: float, final: bool=False):
    slim_results = []
    for r in all_results:
        slim = {k: v for k, v in r.items() if k != 'per_fold'}
        slim['per_fold_f1s'] = [f['best_val_f1'] for f in r['per_fold']]
        slim_results.append(slim)
    output = {'search_strategy': args.strategy, 'n_folds': args.n_folds, 'n_trials_run': len(all_results), 'total_time_seconds': time.time() - total_start, 'best_trial': {'trial': best_trial['trial'], 'hyperparams': best_trial['hyperparams'], 'mean_val_f1': best_trial['mean_val_f1'], 'std_val_f1': best_trial['std_val_f1'], 'mean_val_accuracy': best_trial['mean_val_accuracy'], 'mean_val_roc_auc': best_trial['mean_val_roc_auc']} if best_trial else None, 'all_trials': slim_results}
    ...
    if final:
        ...
        with open(lb_path, 'w') as f:
            f.write(f"{'Rank':<5} {'Mean F1':<10} {'± Std':<10} {'Acc':<10} {'AUC':<10} {'LR':<10} {'BS':<5} {'Drop':<6} {'WD':<8} {'Warm':<6} {'Ep':<4} {'Sched':<8} {'MaxL':<5} {'Time':<8}\n")
            f.write('-' * 110 + '\n')
            for rank, r in enumerate(ranked, 1):
                hp = r['hyperparams']
                f.write(f"{rank:<5} {r['mean_val_f1']:<10.4f} {r['std_val_f1']:<10.4f} {r['mean_val_accuracy']:<10.4f} {r['mean_val_roc_auc']:<10.4f} {hp['learning_rate']:<10.1e} {hp['batch_size']:<5} {hp['dropout_rate']:<6} {hp['weight_decay']:<8} {hp['warmup_ratio']:<6} {hp['num_epochs']:<4} {hp['lr_scheduler_type']:<8} {hp['max_length']:<5} {r.get('trial_time_seconds', 0):<8.1f}\n")
...
def _print_summary(all_results: List[Dict], best_trial: Optional[Dict], total_time: float):
    ...
    print(f"  {'Rank':<5} {'Mean F1':<10} {'± Std':<10} {'LR':<10} {'BS':<5} {'Drop':<6} {'WD':<8} {'Sched':<8}")
    print('  ' + '-' * 65)
    for rank, r in enumerate(ranked[:5], 1):
        hp = r['hyperparams']
        print(f"  {rank:<5} {r['mean_val_f1']:<10.4f} {r['std_val_f1']:<10.4f} {hp['learning_rate']:<10.1e} {hp['batch_size']:<5} {hp['dropout_rate']:<6} {hp['weight_decay']:<8} {hp['lr_scheduler_type']:<8}")
    print('=' * 70)
