...
def main():
    ...
    train_dataset, val_dataset, test_dataset, tokenizer = create_topic_datasets(args.data_path, config, args.test_path)
    train_loader = DataLoader(train_dataset, batch_size=config.batch_size, shuffle=True)
    val_loader = DataLoader(val_dataset, batch_size=config.batch_size, shuffle=False)
    test_loader = DataLoader(test_dataset, batch_size=config.batch_size, shuffle=False)
    label_counts = Counter(train_dataset.labels)
    total = len(train_dataset.labels)
    # weights keep the topic loss fair if one topic is smaller
    ...
    for epoch in range(config.num_epochs):
        ...
        if is_best:
            # best topic model is chosen by validation macro F1
    ...
    summary = {'training_summary': {'stage': 'stage1_topic', 'total_epochs_configured': config.num_epochs, 'epochs_actually_run': actual_epochs_run, 'stopped_early': stopped_early, 'early_stopping_patience': early_stop_patience, 'best_epoch': max(training_log['epochs'], key=lambda e: e['val_metrics']['macro_f1'])['epoch'], 'best_val_f1': best_val_f1, 'final_test_f1': test_metrics['macro_f1'], 'final_test_accuracy': test_metrics['accuracy'], 'total_training_time': training_log['total_duration_seconds']}, 'per_epoch_summary': [{'epoch': ep['epoch'], 'train_loss': ep['train_loss'], 'train_f1': ep['train_metrics']['macro_f1'], 'train_acc': ep['train_metrics']['accuracy'], 'val_f1': ep['val_metrics']['macro_f1'], 'val_acc': ep['val_metrics']['accuracy'], 'val_loss': ep['val_metrics']['loss'], 'is_best': ep['is_best'], 'duration': ep['duration_seconds']} for ep in training_log['epochs']]}
