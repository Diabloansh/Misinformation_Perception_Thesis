...
def format_results_log(results_dir: str) -> str:
    ...
    lines.append(f"  Total duration:     {training_log['total_duration_seconds']:.1f} seconds ({training_log['total_duration_seconds'] / 60:.1f} minutes)")
    lines.append('')
    ...
    lines.append(sep)
    header_ep = f"  {'Epoch':>5}  {'Train Loss':>11}  {'Train F1':>9}  {'Train Acc':>10}  {'Val F1':>7}  {'Val Acc':>8}  {'Val Loss':>9}  {'Best':>5}  {'Time(s)':>8}"
    lines.append(header_ep)
    lines.append('  ' + '-' * 90)
    for ep in epochs:
        best_marker = '  *' if ep['is_best'] else ''
        lines.append(f"  {ep['epoch']:>5}  {ep['train_loss']:>11.4f}  {ep['train_f1']:>9.4f}  {ep['train_acc']:>10.4f}  {ep['val_f1']:>7.4f}  {ep['val_acc']:>8.4f}  {ep['val_loss']:>9.4f}  {best_marker:>5}  {ep['duration']:>8.1f}")
    lines.append('')
    lines.append(sep)
...
def main():
    parser = argparse.ArgumentParser(description='Generate a human-readable results_log.txt from a main-route training results directory.')
    parser.add_argument('results_dir', nargs='?', default=os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'main_route_classifier', 'results_main'), help='Directory containing training_log_final.json, test_metrics.json, and training_summary.json.')
