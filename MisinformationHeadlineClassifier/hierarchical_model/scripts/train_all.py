...
def main():
    ...
    stage1_cmd = [python, 'src/train_stage1.py', '--data_path', args.data, '--output_dir', stage1_dir, '--epochs', str(args.epochs), '--batch_size', str(args.batch_size), '--learning_rate', str(args.learning_rate), '--patience', str(args.patience)]
    if args.test_data:
    ...
    stage2_health_cmd = [python, 'src/train_stage2.py', '--data_path', args.data, '--topic', 'health', '--output_dir', stage2_health_dir, '--epochs', str(args.epochs), '--batch_size', str(args.batch_size), '--learning_rate', str(args.learning_rate), '--patience', str(args.patience)]
    if args.test_data:
    ...
    stage2_tech_cmd = [python, 'src/train_stage2.py', '--data_path', args.data, '--topic', 'technology', '--output_dir', stage2_tech_dir, '--epochs', str(args.epochs), '--batch_size', str(args.batch_size), '--learning_rate', str(args.learning_rate), '--patience', str(args.patience)]
    if args.test_data:
    ...
    eval_cmd = [python, 'src/evaluate.py', '--data_path', args.data, '--stage1_model', os.path.join(stage1_dir, 'best_model.pt'), '--stage2_health_model', os.path.join(stage2_health_dir, 'best_model.pt'), '--stage2_tech_model', os.path.join(stage2_tech_dir, 'best_model.pt'), '--output_dir', eval_dir]
