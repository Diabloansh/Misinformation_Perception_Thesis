...
def predict(text: str, model, tokenizer, config, device):
    ...
    return {'text': text, 'predicted_label': config.label_names[predicted_class], 'confidence': float(probs[predicted_class]), 'probabilities': {name: float(probs[i]) for i, name in enumerate(config.label_names)}}
