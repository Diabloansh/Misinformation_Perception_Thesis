...
def main():
    ...
    parser.add_argument('--topic', type=str, required=True, choices=['health', 'technology'], help='Topic to train route classifier for')
    ...
    train_dataset, val_dataset, test_dataset, tokenizer = create_route_datasets(args.data_path, config, args.topic, args.test_path)
    ...
    for i in range(config.num_labels):
        ...
        if count == 0:
            # a topic split can miss a route class
            weights.append(0.0)
        else:
            weights.append(total / (config.num_labels * count))
    ...
    for epoch in range(config.num_epochs):
        ...
        if is_best:
            # save the best route model for this topic
    ...
    summary = {'training_summary': {'stage': f'stage2_{args.topic}', 'topic': args.topic, 'total_epochs_configured': config.num_epochs, 'epochs_actually_run': actual_epochs_run, 'stopped_early': stopped_early, 'early_stopping_patience': early_stop_patience, 'best_epoch': max(training_log['epochs'], key=lambda e: e['val_metrics']['macro_f1'])['epoch'], 'best_val_f1': best_val_f1, 'final_test_f1': test_metrics['macro_f1'], 'final_test_accuracy': test_metrics['accuracy'], 'total_training_time': training_log['total_duration_seconds']}, 'per_epoch_summary': [{'epoch': ep['epoch'], 'train_loss': ep['train_loss'], 'train_f1': ep['train_metrics']['macro_f1'], 'train_acc': ep['train_metrics']['accuracy'], 'val_f1': ep['val_metrics']['macro_f1'], 'val_acc': ep['val_metrics']['accuracy'], 'val_loss': ep['val_metrics']['loss'], 'is_best': ep['is_best'], 'duration': ep['duration_seconds']} for ep in training_log['epochs']]}
