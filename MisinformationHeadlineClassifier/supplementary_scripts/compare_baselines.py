...
def main():
    ...
    prior_metrics = class_prior_baseline_metrics(train_labels, test_labels, config.num_labels, config.label_names)
    mdl_metrics = model_metrics(args.model, test_texts, test_labels, config, device)
    os.makedirs(args.output, exist_ok=True)
    for name, base, fname in [('Random Baseline', rand_metrics, 'random_baseline_comparison.json'), ('Class-Prior Baseline', prior_metrics, 'class_prior_baseline_comparison.json')]:
        comparison = {'baseline_name': name, 'test_set_size': len(test_labels), 'baseline_metrics': base, 'model_metrics': mdl_metrics, 'improvements': {'macro_f1_diff': mdl_metrics['macro_f1'] - base['macro_f1'], 'accuracy_diff': mdl_metrics['accuracy'] - base['accuracy'], 'roc_auc_diff': mdl_metrics['roc_auc'] - base['roc_auc']}}
        ...
        for m in ['macro_f1', 'accuracy', 'roc_auc']:
            print(f'  {m:<18} {base[m]:>10.4f} {mdl_metrics[m]:>10.4f} {mdl_metrics[m] - base[m]:>+10.4f}')
