...
from utils import setup_logging, compute_metrics, print_classification_report, save_metrics
...
def main():
    ...
    parser.add_argument('--stage2_health_model', type=str, required=True, help='Path to stage2 health best_model.pt')
    parser.add_argument('--stage2_tech_model', type=str, required=True, help='Path to stage2 technology best_model.pt')
    parser.add_argument('--output_dir', type=str, default='results/hierarchical_eval', help='Output directory for metrics')
    ...
    stage2_health_model, stage2_health_config, stage2_health_tokenizer = load_trained_model(args.stage2_health_model, device)
    stage2_tech_model, stage2_tech_config, stage2_tech_tokenizer = load_trained_model(args.stage2_tech_model, device)
    ...
    for item in data_list:
        ...
        route_label = None
        # convert one-hot route fields into one route id
    ...
    topic_dataset = HeadlineDataset(texts, true_topics.tolist(), stage1_tokenizer, stage1_config.max_length)
    ...
    if health_mask.any():
        # run only predicted health rows through the health route model
        ...
        health_dataset = HeadlineDataset(health_texts, health_labels, stage2_health_tokenizer, stage2_health_config.max_length)
    ...
    if tech_mask.any():
        # same routing idea for technology rows
        ...
        tech_dataset = HeadlineDataset(tech_texts, tech_labels, stage2_tech_tokenizer, stage2_tech_config.max_length)
    ...
    # topic errors matter because they send examples to the wrong route model
    topic_errors = (pred_topics != true_topics).sum()
    combined = {'stage1_topic_accuracy': float(topic_metrics['accuracy']), 'stage1_topic_f1': float(topic_metrics['macro_f1']), 'stage2_route_accuracy': float(route_metrics['accuracy']), 'stage2_route_f1': float(route_metrics['macro_f1']), 'stage2_route_roc_auc': float(route_metrics['roc_auc']), 'topic_misclassifications': int(topic_errors), 'total_samples': len(texts)}
