...
def compute_metrics(probabilities: np.ndarray, true_labels: np.ndarray, label_names: List[str]) -> Dict:
    ...
    precision, recall, f1, support = precision_recall_fscore_support(true_labels, pred_labels, labels=class_labels, average=None, zero_division=0)
    macro_precision, macro_recall, macro_f1, _ = precision_recall_fscore_support(true_labels, pred_labels, labels=class_labels, average='macro', zero_division=0)
    ...
    try:
        ...
        roc_auc = 0.0
    cm = confusion_matrix(true_labels, pred_labels, labels=class_labels).tolist()
...
def print_classification_report(probabilities: np.ndarray, true_labels: np.ndarray, label_names: List[str]):
    pred_labels = probabilities.argmax(axis=1)
    print(classification_report(true_labels, pred_labels, labels=list(range(len(label_names))), target_names=label_names, digits=4, zero_division=0))
