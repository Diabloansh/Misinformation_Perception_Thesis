...
def main():
    ...
    train_dataset, val_dataset, test_dataset, tokenizer = create_datasets(args.data_path, config, args.test_path)
    train_loader = DataLoader(train_dataset, batch_size=config.batch_size, shuffle=True)
    val_loader = DataLoader(val_dataset, batch_size=config.batch_size, shuffle=False)
    test_loader = DataLoader(test_dataset, batch_size=config.batch_size, shuffle=False)
    label_counts = Counter(train_dataset.labels)
    total = len(train_dataset.labels)
    # inverse frequency helps when one class is smaller
    class_weights = torch.tensor([total / (config.num_labels * label_counts[i]) for i in range(config.num_labels)], dtype=torch.float32)
    ...
    training_log = {'start_time': datetime.now().isoformat(), 'config': {'model_name': config.model_name, 'num_labels': config.num_labels, 'num_epochs': config.num_epochs, 'batch_size': config.batch_size, 'learning_rate': config.learning_rate, 'weight_decay': config.weight_decay, 'max_length': config.max_length, 'warmup_ratio': config.warmup_ratio, 'lr_scheduler': config.lr_scheduler_type, 'dropout_rate': config.dropout_rate, 'early_stopping_patience': args.patience}, 'epochs': [], 'device': str(device), 'dataset_sizes': {'train': len(train_dataset), 'val': len(val_dataset), 'test': len(test_dataset)}}
    best_val_f1 = 0.0
    epochs_no_improve = 0
    ...
    for epoch in range(config.num_epochs):
        ...
        torch.save({'epoch': epoch + 1, 'model_state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict(), 'scheduler_state_dict': scheduler.state_dict(), 'train_loss': train_loss, 'val_metrics': val_metrics, 'train_metrics': train_metrics, 'config': config, 'tokenizer': tokenizer}, epoch_path)
        if is_best:
            # save the model with the best validation macro F1
            best_val_f1 = val_metrics['macro_f1']
            torch.save({'model_state_dict': model.state_dict(), 'config': config, 'tokenizer': tokenizer, 'epoch': epoch + 1, 'val_metrics': val_metrics}, config.model_save_path + '.pt')
            logging.info(f'  *** New best model saved  (Val F1 = {best_val_f1:.4f}) ***')
        epoch_log = {'epoch': epoch + 1, 'timestamp': epoch_start.isoformat(), 'duration_seconds': epoch_duration, 'train_loss': train_loss, 'train_metrics': train_metrics, 'val_metrics': val_metrics, 'learning_rate': optimizer.param_groups[0]['lr'], 'is_best': is_best}
    ...
    # test only after loading the best checkpoint
    ...
    summary = {'training_summary': {'total_epochs_configured': config.num_epochs, 'epochs_actually_run': actual_epochs_run, 'stopped_early': stopped_early, 'early_stopping_patience': early_stop_patience, 'best_epoch': max(training_log['epochs'], key=lambda e: e['val_metrics']['macro_f1'])['epoch'], 'best_val_f1': best_val_f1, 'final_test_f1': test_metrics['macro_f1'], 'final_test_accuracy': test_metrics['accuracy'], 'total_training_time': training_log['total_duration_seconds']}, 'per_epoch_summary': [{'epoch': ep['epoch'], 'train_loss': ep['train_loss'], 'train_f1': ep['train_metrics']['macro_f1'], 'train_acc': ep['train_metrics']['accuracy'], 'val_f1': ep['val_metrics']['macro_f1'], 'val_acc': ep['val_metrics']['accuracy'], 'val_loss': ep['val_metrics']['loss'], 'is_best': ep['is_best'], 'duration': ep['duration_seconds']} for ep in training_log['epochs']]}
